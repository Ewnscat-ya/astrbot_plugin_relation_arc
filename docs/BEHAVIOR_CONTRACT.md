...
- **注入生命周期**：幂等（重复 inject 先剥离本插件旧块再追加，不叠加、动态数据刷新）；禁用时清除本插件上一轮注入；只管理自有标记块——用户文本、相似示例、其他插件内容不动。宿主路径核实（非仅 mark_as_temp 断言）：`assemble_context()` 把临时部件并入本轮用户消息；`dump_messages_with_checkpoints()` 落历史时按部件级 `_no_save` 过滤（宿主契约测试覆盖）。
- **首行协议不变**：仍要求首行且仅一个 `<relation_judgment>`、schema 3、随后自然正文；未采纳上游末行标签。
- **离线对照（字符，非 token）**：同 persona 下稳定前缀 10→1204 字符（固定块 1192，SHA256 跨场景一致）；每轮开销 1831→839 / 1903→953；总开销 +200 字符左右（约 +11%）。无 tokenizer、无缓存命中率、无费用结论——线上收益待真实模型对照（MIS-161）。
