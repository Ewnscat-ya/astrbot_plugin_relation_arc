...
| 虚拟环境 + astrbot 4.28.0 | `python -m unittest discover -s tests` | **164/164 通过**（8.5s） |
| 虚拟环境 + astrbot 4.26.0（声明下限） | 同上 | **164/164 通过**（8.2s） |
...
3. 在仓库目录运行：`python -m unittest discover -s tests -v` → 应为 `Ran 164 tests ... OK`。
