...
| 虚拟环境 + astrbot 4.28.0 | `python -m unittest discover -s tests` | **176/176 通过**（22.4s） |
| 虚拟环境 + astrbot 4.26.0（声明下限） | 同上 | **176/176 通过**（24.6s） |
...
3. 在仓库目录运行：`python -m unittest discover -s tests -v` → 应为 `Ran 176 tests ... OK`。
