...
## 测试执行结果（144 个方法：核心 51 / 入口 71 / P0 9 / 宿主兼容 13）

92（MIS-87 基线）→ 99（MIS-89 新增 7 个版本门禁与身份修复回归，复现先行）→ 112（MIS-88 新增 13 个宿主兼容回归，走宿主真实 Quart 兼容桥）→ 126（MIS-90 新增 14 个群定向与轮次上下文回归，复现先行：修复前 7 项失败）→ 137（MIS-91 新增 11 个协议预算/截断契约/富消息顺序回归）→ 144（MIS-92 新增 7 个并发原子性/幂等/排他/故障注入回归；并发差距以旧代码结构事实为准——事务外窗口读无法在单进程确定性复现，已随旧路径删除并记录）。现有方法零改动、零删除。
...
| 虚拟环境 + astrbot 4.28.0 | `python -m unittest discover -s tests` | **144/144 通过**（5.3s） |
| 虚拟环境 + astrbot 4.26.0（声明下限） | 同上 | **144/144 通过**（5.3s） |
...
3. 在仓库目录运行：`python -m unittest discover -s tests -v` → 应为 `Ran 144 tests ... OK`。
