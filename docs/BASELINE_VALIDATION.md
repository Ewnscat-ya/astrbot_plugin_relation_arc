...
| 虚拟环境 + astrbot 4.28.0 | `python -m unittest discover -s tests` | **169/169 通过**（20.1s） |
| 虚拟环境 + astrbot 4.26.0（声明下限） | 同上 | **169/169 通过**（18.1s） |
...
3. 在仓库目录运行：`python -m unittest discover -s tests -v` → 应为 `Ran 169 tests ... OK`。
