...
| 项 | 值 | 说明 |
|---|---|---|
| 插件版本 | 0.2.0（metadata.yaml） | v0.2.0：提示布局重构（MIS-158～162），见 §8 |
...
## 8. 提示布局重构（v0.2.0，MIS-158～162）

固定规则块入 system 尾部（人设在前）、动态状态与短提醒保持每轮临时部件、注入幂等且禁用即清理、effects 白名单移交动态块。结算语义零变化（冻结样例一致）；离线对照见 `tools/prompt_samples/eval_report.json`（等价共享输入：可见性×有效排他；稳定前缀 10→1204 字符、固定块哈希跨场景一致、每轮临时开销 1832~1928→840~953、总开销 1832~1928→2032~2145 约 +9%~+11%——两侧同计固定块与动态块，字符非 token，无缓存/费用结论）。两侧完整请求样例并排留存：`tools/prompt_samples/baseline_*.json`（913ca59 基线，同口径 7 场景——system 不含插件块、每轮 2 个临时部件、重复注入叠加、禁用后残留）与候选同名 fixtures（固定块入 system、每轮恰好 动态块+短提醒 两部件、重复注入幂等、禁用即清空）；另有 **full_request 级证据**（U03）：`full_request_*.json` / `baseline_full_request_*.json` 各 7 组——记录合成用户原文/预设历史/1×1 数据 URI 图片/其他插件部件输入、inject 后请求、**真实宿主 assemble_context 组装结果**与 **dump_messages_with_checkpoints 历史过滤结果**（临时块不落历史、用户原文/外部部件/图片块保留），含生成 commit 与宿主版本；宿主测试同样驱动真实 inject→组装→历史链。线上收益待真实模型对照（MIS-161 未完成）。回退：revert 布局提交即可，无数据迁移。

## 9. v0.2.1 接手修复

U01–U05 的独立接手结论与最终证据见 [本轮交付](TAKEOVER_DELIVERY_20260918.md)。上方 v0.2.0 的旧测试数字与旧请求文件保留为历史记录，不代表 v0.2.1 验收。

当前可运行入口见 [模型对照与离线采集](MODEL_AB.md)：必须使用显式 `--repo` 指定两侧源码。旧版“自动 Provider”和切换 checkout 的示例已经被替代。
