...
固定规则块入 system 尾部（人设在前）、动态状态与短提醒保持每轮临时部件、注入幂等且禁用即清理、effects 白名单移交动态块。结算语义零变化（冻结样例一致）；离线对照见 `tools/prompt_samples/eval_report.json`（稳定前缀 10→1204 字符、固定块哈希跨场景一致、每轮开销下降、总开销约 +11%——字符非 token，无缓存/费用结论）。线上收益待真实模型对照（MIS-161 未完成）。回退：revert 布局提交即可，无数据迁移。
