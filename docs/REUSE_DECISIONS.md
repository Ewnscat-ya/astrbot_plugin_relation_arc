...
## MIS-158 布局借鉴记录（2026-09-14）

- 借鉴对象：Favour Ultra `2f2ec141`（v4.4.5）main.py:2139-2202 的动静分离注入（PART A 固定规则追加 system_prompt、人设在前机制在后；PART B 动态数据临时注入）与 v4.4.4 的 MandatoryFooter 尾部近因短提醒思路、v4.0.0 的动静分离动机。
- 采纳：布局结构（固定前置/动态后置/当前轮短提醒）与"人设在前、机制在后"注入顺序。
- 不采纳：末行标签协议（保留首行 `<relation_judgment>`）、单维好感模型、其关系标签语法、门槛矩阵注入、任何玩法语义。
- 实现为本仓库独立文本（prompts.py 重写），非文件级复制。
