...
from .relation_engine import DIMENSIONS, DEFAULT_VALUES, clamp
...
class RelationStore:
    ...
    def record_protocol_health(self, outcome: str, source: str, bare_recovery: bool, effect_count: int, retention_days: int) -> None:
        if outcome not in {"missing_protocol","invalid_protocol","model_no_effects","invalid_effects","zero_after_policy","applied","skipped_group_not_directed","skipped_no_stable_message_id"} or source not in {"completion_text","result_chain","none"}: return
        with self.lock, self._connection() as conn:
    ...
    def set_dimension(self, identity: str, scope_kind: str, scope_id: str, dimension: str, value: int, actor: str = "administrator") -> dict[str, int]:
        ...
        values = self.account(identity, scope_kind, scope_id)["values"]
        old = values.get(dimension, 0)
        return self.apply(event_id=f"{actor}:{identity}:{dimension}:{time.time_ns()}", identity=identity, scope_kind=scope_kind, scope_id=scope_id, source_kind="admin", evidence="", reason="manual adjustment", requested={dimension: value - old}, applied={dimension: value - old}, notes={}, actor=actor)
