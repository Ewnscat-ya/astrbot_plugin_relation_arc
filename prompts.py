"""Prompt construction for Relation Arc (MIS-100 split; wired into the real
inject chain in MIS-127 R4). Pure text builders: every value the model sees
is passed in explicitly by the caller. The numeric gates are enforced by the
backend at settlement time and are no longer injected as a per-turn matrix -
the same turn's score can still cross a line, so all five candidate types
are always listed."""
...
def binding_context(binding_labels: list[str], exclusivity_on: bool, exclusive_occupied: bool) -> str:
    """R4: the model only ever sees its own relation labels. The cross-user
...
def types_directory_line() -> str:
    """R4: the fixed five legal keys and Chinese names, without the numeric
    gate matrix; the backend decides admission at settlement."""
    return "可选 type_id（固定五类）：" + "、".join(
        item["key"] + "（" + item["label"] + "）" for item in public_directory()) + "。"


def upgrade_note() -> str:
    return ("若当前已存在恋人关系，后端会把合法的 bind/spouse 提案识别为关系升级并原子完成；"
            "是否建立、升级或拒绝一律以结算结果为准，不要把高分或尚未通过后端的提案写成已经确立的事实。")
